...
class UnstructuredRSTParser(_UnstructuredDocumentParseur):
    ...
    def _get_elements_via_local(self, blob:Blob) -> list:
        try:
            from unstructured.partition.rst import partition_rst  # type: ignore
        except ImportError:
            raise ImportError(
                "unstructured package not found, please install it with "
                "`pip install 'langchain_unstructured[rst]'`"
            )

        if blob.data is None and blob.path:
            return partition_rst(
                filename=blob.path,
